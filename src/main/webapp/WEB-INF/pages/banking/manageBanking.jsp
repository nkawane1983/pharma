				...
				<div class="col-md-12">
					...
					<div class="card card-block">
						...
						<div class="col-md-12">
							...
							<c:choose>
								<c:when
									test="${not empty banking && not empty banking.getId() && (not empty mode ||not empty report)}">
							...
							<c:if test="${ ( empty mode && empty report) }">
								<div class="col-sm-2 col-xs-12" style="text-align: right;">
									<label class="control-label" for="formGroupExampleInput2">Total
										Cash banked</label>
							...
							<c:if test="${(not empty mode ||not empty report) }">
								<div class="col-sm-2 col-xs-12" style="text-align: right;">
									<label class="control-label" for="formGroupExampleInput2">Total
										Cash banked</label>
									<form:input path="cash" class="form-control" id="cash"
									...
									<c:choose>
										<c:when test="${not empty report}">
											...
											<section class="section pull-right">
												<br /> <a
													href="${pageContext.request.contextPath}/report/bankingMonthlyReportPg.do?branchId=${branchid}"
													type="button" class="btn btn-secondary"><i class="fa fa-arrow-left"> &nbsp;Back</i></a> <input
													type="hidden" id="reportid" value="${report}">
													<input type="hidden" id="mode" value="${report}" />
...
<script>
	...
	function cashCalculate(cash) {
		...
		if (str1[0] == 'qutP') {
			...
			document.getElementById('amtP_' + str1[1]).value = val1;
	...
	function setallvalue() {
		...
		$('#totalbanking').val(
				(parseFloat($('#cash').val()) + totalchequ + totalpaid)
						.toFixed(2));
	...
	function carryForwrd() {
		...
		totalbank = (parseFloat(totalcashbank) + parseFloat($('#totalCheques').val()) + parseFloat($('#miscCash').val()));
		if (isNaN(totalbank))
			totalbank = 0.0;
		$('#totalbanking').val(totalbank.toFixed(2));
		if((totalcashobj + outsatanding)<totalcashbank ||$('#cash').val()=='')
	...
	function carryForwrdcash() {
		...
		totalbank = (parseFloat(totalcashbank)
				+ parseFloat($('#totalCheques').val()) + parseFloat($(
				'#miscCash').val()));
		if (isNaN(totalbank))
			totalbank = 0.0;
		//alert(totalbank)
